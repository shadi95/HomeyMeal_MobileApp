...
class HtmlViewerScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: ResponsiveHelper.isDesktop(context) ? MainAppBar() :  CustomAppBar(context: context, title: getTranslated(htmlType == HtmlType.TERMS_AND_CONDITION ? 'terms_and_condition'
            ...
            children: [
              ResponsiveHelper.isDesktop(context) ? Container(
                ...
                child: SelectableText(getTranslated(htmlType == HtmlType.TERMS_AND_CONDITION ? 'terms_and_condition'
                    : htmlType == HtmlType.ABOUT_US ? 'about_us' : htmlType == HtmlType.PRIVACY_POLICY ? 'privacy_policy' : 'no_data_found', context),
                  style: rubikBold.copyWith(fontSize: Dimensions.FONT_SIZE_EXTRA_LARGE, color: Colors.black),
                ),
              ) : SizedBox(),
              SizedBox(height: 30),
